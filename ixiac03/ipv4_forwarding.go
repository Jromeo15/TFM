...
- Endpoints: OTG 1.1.1.1 -----> 1.1.1.2 (srl1) 10.10.10.1 ------> 10.10.10.2 (srl2) 2.2.2.2 ------> OTG 2.2.2.1
- Static Route on srl1: 20.20.20.0/24 -> 10.10.10.2
- Static Route on srl2: 20.20.20.0/24 -> 2.2.2.1
- Static Route on srl1: 2.2.2.0/24 -> 10.10.10.2
- Static Route on srl2: 1.1.1.0/24 -> 10.10.10.1
...
func newConfig() (gosnappi.GosnappiApi, gosnappi.Config) {
    ...
    // create a flow between r2 and r1 (reverse direction)
    f2 := config.Flows().Add().SetName("r2.v4.r1")
    f2.TxRx().Device().SetTxNames([]string{r2Ip.Name()})
    f2.TxRx().Device().SetRxNames([]string{r1Ip.Name()})
    ...
    f2.Rate().SetPps(10)
    f2.Duration().FixedPackets().SetPackets(uint32(pktCount))

    // configure headers for all packets in the flow
    eth = f2.Packet().Add().Ethernet()
    ip = f2.Packet().Add().Ipv4()
    tcp = f2.Packet().Add().Tcp()

    eth.Src().SetValue(r2Mac)
    eth.Dst().SetChoice("auto")

    ip.Src().SetValue("20.20.20.1")
